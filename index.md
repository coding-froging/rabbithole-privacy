...
- Saved cards and your settings (interests, languages, optional API keys) are stored only on your device. API keys are kept in the iOS Keychain.
- The "taste" model — what you swiped left on, saved, opened or lingered on — is a small set of numbers stored only on your device. You can see, edit and reset it in Settings → Taste. It never leaves the phone.
- Images are cached on the device to save traffic.
